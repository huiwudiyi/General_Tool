...
def load_tools_from_json(path: str) -> List[Dict[str, Any]]:
    """读取 tool 集合 JSON 文件。"""
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, list):
        raise ValueError("tools json must be a list of dict")
    return data


def load_query_gold_from_json(path: str) -> Dict[str, List[str]]:
    """读取 query 测试集合 JSON 文件。"""
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, dict):
        raise ValueError("query json must be a dict: {query: [gold_id1, gold_id2]}")
    return {str(k): ToolPassAtKRecallEvaluator._normalize_gold_ids(v) for k, v in data.items()}


def main():
    """示例主函数：你可以替换为自己的 JSON 文件路径。"""
    tools = [
        {"id": "idx1", "title": "工具1", "description": "这是工具1的描述"},
        {"id": "idx2", "title": "工具2", "description": "这是工具2的描述"},
        {"id": "idx3", "title": "工具3", "description": "这是工具3的描述"},
        {"id": "idx4", "title": "工具4", "description": "这是工具4的描述"},
    ]
    query_gold_ids = {
        "query2": ["idx1", "idx4"],
    }
    # 方式 1：直接写入 Python 变量
    tools = []
    with open("data/tool_descriptions.json") as f:
        tools =  json.load(f)
        query_gold_ids = {
            "query2": ["idx1", "idx4"],
        }
    ...
    # 方式 2：从 JSON 文件读取
    query_gold_ids = {}
    with open("data/high_pv_query.json") as f:
        query_gold_ids = json.load(f)
