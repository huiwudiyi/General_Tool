...
def _bypass_proxy_for(url: str) -> None:
    """把内网/本机地址加入 no_proxy，避免被环境里的 http_proxy 转发导致 502。"""
    hostname = urlparse(url).hostname
    if not hostname:
        return
    for key in ("no_proxy", "NO_PROXY"):
        value = os.environ.get(key, "")
        if hostname not in value.split(","):
            os.environ[key] = f"{hostname},{value}".rstrip(",")


class LLMClient:
    """OpenAI-compatible chat-completions client."""
    def __init__(self, base_url, api_key, timeout=60, max_retry=6, host=None, appid=None,
                 sleep_seconds=2, max_backoff=60):
        self.base_url = base_url
        self.api_key = api_key
        self.max_retry = max_retry
        # 退避基数与上限：并行请求时用指数退避 + 随机抖动错峰
        self.sleep_seconds = sleep_seconds
        self.max_backoff = max_backoff
        self.timeout = timeout
        _bypass_proxy_for(base_url)
            ...
            for attempt in range(1, self.max_retry + 1):
                try:
                    ...
                    if attempt < self.max_retry:
                        # 指数退避 + 随机抖动，适配并行场景下的错峰
                        backoff = min(self.sleep_seconds * (2 ** (attempt - 1)), self.max_backoff)
