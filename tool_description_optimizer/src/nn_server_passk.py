...
# 环境中的 http_proxy 会把 127.0.0.1 的请求也转发到公司代理（返回 502），
# 因此在创建客户端前确保本机地址走直连。
...
# 初始化客户端，对接本机 GPU2 上的 SGLang embedding 服务
EMB_CLIENT = OpenAI(
    base_url="http://127.0.0.1:8882/v1",  # 末尾必须带 /v1
    api_key="zacharychu"                   # 和你启动命令里 --api-key 保持一致
)

def encode_fn(texts: List[str], BATCH_SIZE=64, normalize_embeddings=False,) -> np.ndarray:
    all_embeddings = []
    # tqdm 全局进度条
    total = len(texts)
