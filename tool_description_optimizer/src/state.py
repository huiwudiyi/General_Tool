...
class ToolOptimizerState(TypedDict):
    ...
    critic_history: list[InfoRecord]
    generate_history: list[InfoRecord]
...
def append_critic_history(state: ToolOptimizerState, record: InfoRecord) -> list[InfoRecord]:
    """追加版本快照至历史仓库，不修改原有记录"""
    return [*state["critic_history"], record]

def append_generate_history(state: ToolOptimizerState, record: InfoRecord) -> list[InfoRecord]:
    """追加版本快照至历史仓库，不修改原有记录"""
    return [*state["generate_history"], record]
