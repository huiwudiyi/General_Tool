...
# 初始化客户端，对接本地SGLang服务
EMB_CLIENT = OpenAI(
    base_url="http://127.0.0.1:8881/v1",  # 末尾必须带 /v1
    api_key="zacharychu"                   # 和你启动命令里 --api-key 保持一致
)

def encode_fn(texts: List[str], normalize_embeddings=False) -> np.ndarray:
    """
    入参：仅texts列表；batch逻辑内部自管理，自动降级重试，解决SGLang 500
    """
    all_embeddings = []
    total = len(texts)
    pbar = tqdm(total=total, desc="向量编码")
    idx = 0
    base_batch = 4
    ...
    while idx < total:
        ...
        batch_ok = False
        batch_texts = texts[idx: idx + current_batch]
        # 重试循环，500自动减半batch
        for retry in range(3):
            try:
                ...
                if e.status_code == 500:
                    wait = 2 ** retry
                    print(f"[WARN 500] batch失败，重试{retry+1}, sleep {wait}s，缩小batch: {current_batch} -> {max(1, current_batch//2)}")
                    time.sleep(wait)
                    ...
                    raise
        if not batch_ok:
            raise RuntimeError(f"文本片段编码彻底失败，起始位置idx={idx}")
    pbar.close()
...
# def encode_fn(texts: List[str], BATCH_SIZE=4, normalize_embeddings=False,) -> np.ndarray:
#     all_embeddings = []
#     # tqdm 全局进度条
#     total = len(texts)
#     pbar = tqdm(total=total, desc="向量生成进度")
#     print("-" * 66)
#     texts = ["放哪地方"] * 66
#     print(total, texts)
#     # 分片循环调用
...
#         # time.sleep(0.01)
#     pbar.close()
#     all_embeddings = l2_normalize(np.asarray(all_embeddings))
#     print("*" * 66)
...
class ToolPassAtKRecallEvaluator:
    ...
    def _batch_encode(self, texts: List[str]) -> np.ndarray:
        """调用encode_fn，内部自动分片+重试，不再外层循环batch"""
        if not texts:
            return np.empty((0, 0), dtype=np.float32)
        emb = self.encode_fn(texts)
        if not isinstance(emb, np.ndarray):
            emb = np.asarray(emb)
        embeddings = l2_normalize(emb)
        return embeddings
    ...
    def calc_pass_at_k(
        ...
        """计算单条 query 的 pass@k / MRR@k / MAP@k。

        指标定义（均只在 top-k 范围内统计）：
        - pass@k：top-k 中命中任意一个 gold 记 1，否则记 0。
        - reciprocal_rank（RR@k）：首个命中的排名倒数 1 / rank；top-k 内无命中记 0。
        - average_precision（AP@k）：
              AP@k = (1 / min(R, k)) * Σ_{i=1..k} [rank i 命中] * Precision@i
          其中 R 为该 query 的 gold 数量，Precision@i 为 top-i 中命中数 / i。
          MAP@k 即所有 query 的 AP@k 求平均。
        """
        normalized_gold_ids = set(ToolPassAtKRecallEvaluator._normalize_gold_ids(gold_ids))
        num_relevant = len(normalized_gold_ids)
        ...
        average_precision = (
            precision_sum / min(num_relevant, k) if num_relevant > 0 else 0.0
        )
    ...
    def evaluate(
        ...
        for i, query in enumerate(queries):
            ...
            for view_name in eval_views:
                ...
                for k in k_list:
                    ...
                    # MRR@k / MAP@k：仅统计有 gold 的 query，避免空 gold 拉低均值
                    if metric["num_relevant"] > 0:
                        stat[key]["reciprocal_rank_sum"] += metric["reciprocal_rank"]
                        stat[key]["average_precision_sum"] += metric["average_precision"]
                        stat[key]["rank_query_count"] += 1
        ...
        for (view_name, k), value in stat.items():
            ...
            rank_query_count = value["rank_query_count"]
            ...
            mrr_at_k = (
                value["reciprocal_rank_sum"] / rank_query_count
                if rank_query_count > 0
                else 0.0
            )
            map_at_k = (
                value["average_precision_sum"] / rank_query_count
                if rank_query_count > 0
                else 0.0
            )
...
def recall_passk_function(tools, query_gold_ids, srcid, output_dir):
...
def main():
    ...
    evaluator.save_recall_results(
        summary_df=summary_df,
        detail_df=detail_df,
        output_dir="../data/tmp/recall_outputs/",
        prefix="tool_pass_at_k_recall",
    )
