...
class ToolOptimizerState(TypedDict):
    ...
    judge_history: list[InfoRecord]
    verify_judge_history: list[InfoRecord]
    # ① Retriever（冻结 embedding）逐轮检索指标，如 mrr@k / pass@k
    retriever_history: list[InfoRecord]
    # ② LLM Selector（冻结裁判）逐轮选择正确率与边界判别结果
    selector_history: list[InfoRecord]
    # 各阶段输出缓存，调试追溯
    optimizer_history: list[InfoRecord]

    # 当轮 GRPO group 的可序列化快照。
    # 之所以要放进 state 而不是留在实例属性里：LangGraph 的 checkpointer 只恢复 state，
    # group 留在 self.group_samples 的话，跨进程从中间节点续跑会拿到空 group、静默产出空结果。
    group_samples: list[dict]
    # sample_index(str) -> {query: [topk srcid]}，Selector 复用检索结果，避免重复检索
    sample_topk: dict
...
def append_verify_judge_history(state: ToolOptimizerState, record: InfoRecord) -> list[InfoRecord]:
    """追加复核评审记录，不修改原有记录"""
    return [*state.get("verify_judge_history", []), record]

def append_retriever_history(state: ToolOptimizerState, record: InfoRecord) -> list[InfoRecord]:
    """追加 Retriever 检索指标记录，不修改原有记录"""
    return [*state.get("retriever_history", []), record]

def append_selector_history(state: ToolOptimizerState, record: InfoRecord) -> list[InfoRecord]:
    """追加 Selector 选择/边界判别记录，不修改原有记录"""
    return [*state.get("selector_history", []), record]
...
def get_version_by_id(state: ToolOptimizerState, vid: int) -> Optional[VersionRecord]:
    """根据版本ID精准查询快照"""
    for record in state["version_history"]:
...
def list_accepted_versions(state: ToolOptimizerState) -> list[VersionRecord]:
    """筛选所有纳入主线的优质版本"""
    return [r for r in state["version_history"] if r.accepted]

def list_rejected_versions(state: ToolOptimizerState) -> list[VersionRecord]:
    """筛选所有被驳回的试验版本"""
    return [r for r in state["version_history"] if not r.accepted]
...
def export_version_history(state: ToolOptimizerState) -> list[dict]:
    ...
    return [rec.to_dict() for rec in state["version_history"]]
