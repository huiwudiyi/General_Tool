...
class ToolOptimizerGraph:
    ...
    def _model_for(self, node_name: str) -> str:
        return self.node_model_map.get(node_name, "doubao-seed-2.0-mini")
    # node -> temperature 
    ...
    def _generate_text(
        ...
        print("prompt", prompt)
        print("model", self._model_for(node_name))
        print("temperature", self._temperature_for(node_name))
...
for path in list_file("../data/data_not_import/summary_output"):
...
dag = ToolOptimizerGraph(resource_id = resource_id,
            ...
            tools_description_path = "../data/data_not_import/summary/tool_descriptions.json",
            test_data_path  = "../data/data_not_import/summary/query.json",
            checkpointer = None)
...
dag._generate_text(node_name = "summary", prompt = prompt)